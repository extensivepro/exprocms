function ArticlesController2($scope, $rootScope, Articles, Pagination, $timeout, $injector,$location){
    /**
    ...
    $scope.saveArticle = function() {
        ...
        if (!$('#title').val()) return;
        var title = $scope.title.replace(/(^\s*)|(\s*$)/g, "").replace(/\s+/g, '-'); //将所有的空白字符变成'-'
       ...
       $scope.articleText = editor.exportFile();
       console.log("title:" + title + " date:" + date + " text:" + $scope.articleText);
       var article = new Article($scope.articleText, title, date);
        Articles.saveArticle(article, function(result) {
            console.log("文章成功提交到后台");
            console.log(result.message);
            window.location.reload();
  /*          $timeout(setCurrentView, 1500);
    ...
    $scope.selectArticle = function(url) {
        console.log("传入的url是：" + url);
        var path = "article/_site" + url;
        console.log("path:" + path);
        $scope.currentViewForT = path;
    }


    // 删除一篇文章
    $scope.remove = function(add, tit) {
        var address = add;
        var title = tit.replace(/\s/g, '-');

        console.log("address:" + address + " title:" + title);
        var re = /%/;
        if (re.test(address)) {
            var v1 = address.indexOf('%');
            var v2 = address.indexOf('.');
            console.log("v1:" + v1 + "v2:" + v2);
            var v3 = address.substring(v1, v2);
            address = address.replace(v3, title);
        ...
        Articles.remove(param, function(result) {
    ...
    // 加载epicEditor
    $scope.showEpic = function() {
      editor = new EpicEditor(opts).load();
    };


    // 在跳转时，获取数据存入rootScope中
    $scope.change = function(add, tit) {
        var address = add;
        var title  = tit;
        $rootScope.titleForC = tit;
        var re = /%/
        // 1.当文件名没有中文时
        if (!re.test(address)) {
            address = address.substring(1, address.length).replace(/\//g, '-').replace(/.html/, '.markdown');
            var add1 = add.substring(1, address.length).replace(/\//g, '-');
            $rootScope.addressForC  = add1.substring(0, 10); //2014-01-02
            $rootScope.titleForC = add1.substring(11, add.length).replace(/.html/, '');
        } else { // 当文件名中有中文时
            ...
            console.log("v1:" + v1 + "v2:" + v2);
            var v3 = address.substring(v1, v2);
            address = address.replace(v3, title).replace(/.html/, '.markdown');
            $rootScope.addressForC  = address.substring(0, v1-1);
        }
        console.log("address:" + address);
        var param = {fileName:address};
        // 通过文件名和日期找到相应的markdown
        Articles.findByAddress(param, function(data) {
            console.log("进入findByAddress data是:" + data.result);
            $rootScope.content = data.result;
            console.log('==============\n' + data.result + '==============');
            $rootScope.currentView.path = "views/article/change.html";
        });
    };

    // 修改页面的初始化
    $scope.initForC = function() {
        console.log("into initForC");
        editor = new EpicEditor(opts).load();
        $scope.titleForC =  $rootScope.titleForC;
        console.log("$rootScope.content:" + $rootScope.content);
        editor.getElement('editor').body.innerText = $rootScope.content;
    };

    // 用户退出修改页面
    $scope.backList = function() {
        console.log("into backList");
        $rootScope.currentView.path = "article/_site/index.html";
    };

    // 保存用户的修改
    $scope.saveArticleForChange = function() {
        var title = $rootScope.titleForC.replace(/\s+/g, '-'); //将所有的空白字符变成'-'
        var date = $rootScope.addressForC;
        function Article(text, title, date) {
            this.text = text;
            this.title = title;
            this.date = date;
        }
        $scope.articleTextForChange = editor.exportFile();
        console.log("title:" + title + " date:" + date + " text:" + $scope.articleTextForChange);
        var article = new Article($scope.articleTextForChange, title, date);
        Articles.saveArticle(article, function(result) {
            console.log("修改文章成功");
            console.log(result.message);
            window.location.reload();
